    ...
    for l in level1:
        if(l['i']<=event.pos[0]<=l['i']+100 and l['j']<=event.pos[1]<=l['j']+100):#看是在哪一個正方形
            if(l['i']==answeri and l['j']==answerj):#如果是正確答案的正方形
                ...
                channel.play(sound[personChosen][1])
                ...
                vol=(10-distance*0.02)*0.1#計算距離和vol大小
                if(vol<0):
                    vol=0
                #printtxt(repr(vol)+" !!!", topicfont, screen, 300, 240,[255, 0, 0])
                if(vol==0):#離太遠放音樂2
...
pygame.display.set_caption("findfindfind")#標題
...
#圖片
mouse_img1='hand1.png'#滑鼠
mouse_img2='hand2.png'
yes_sound=pygame.image.load('yesSound_resize.png')#bgsound圖片
no_sound=pygame.image.load('noSound_resize.png')
start_img=pygame.image.load('start.PNG') #開始頁面的按鈕
...
for i in range(len(person_info)):
    pic.append(pygame.image.load(person_info[i][0]))
    for j in range(3):           
        sound[i][j]=(pygame.mixer.Sound(person_info[i][j+2]))
...
while page:
    for event in pygame.event.get():#獲得事件
        if event.type == QUIT:
            ...
            set_bgsound()
        elif event.type==MOUSEBUTTONDOWN and 150<=event.pos[0]<=250 and 120<=event.pos[1]<=220: #四個不同的人物位置
            personChosen=0
            page=False
    ...
    for i in range(150,550,200):
        for j in range(120,320,180):
            screen.blit(pic[person],[i,j])
            printtxt(person_info[person][1], txtfont, screen, i, j+110,[100, 200, 255])
            person=person+1 
    draw_mouse_hand()
    pygame.display.update()  
...
while page:    
    ...
    for l in level1:#640*480
        pygame.draw.rect(screen, l['color'], (l['i'], l['j'], 100, 100))#Rect(left,top,width,height)
    show_bgsound_pic()
    draw_mouse_hand()
    pygame.display.update()  
...
while page:
    ...
    if(playeri+100>=600 and playerj<=5):#飛到禮物那裏了
        page=False
        
    screen.fill([255,255,255])
    fly(flyUp)#往上飛還是往下飛
    audioData = stream.read(NUM_SAMPLES)  # 將讀入的數據轉為數據組
    audio_data = np.fromstring(audioData, dtype=np.short)  # 計算大於level的取樣數據
    if(np.max(audio_data)>15000):#數據大於15000時代表有聲音輸入，則往右20
        playeri+=20
    else:
        playeri-=20
    ...
    printtxt("Thank you for playing this game", topicfont, screen, 80, 200,[128, 128, 255])
    printtxt("I'm glad that you like it :)", topicfont, screen, 110, 260,[143, 206, 200])
    #將左下角以外的地方畫上長方形遮蔽
    ...
    screen.blit(pic[personChosen],[playeri,playerj])
    screen.blit(gift_img,[580,5])
    show_bgsound_pic()
    pygame.display.update() 
